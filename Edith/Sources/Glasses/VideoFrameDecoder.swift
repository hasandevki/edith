 ...
 *
 * Meta Wearables DAT iOS "CameraAccess" örneğinden alınmıştır
 * (samples/CameraAccess/CameraAccess/Media/VideoFrameDecoder.swift).
 * Sonuna CMSampleBuffer.isHEVCKeyframe() uzantısı eklendi.
 */
...
/// Decompresses HEVC `CMSampleBuffer`s into `UIImage`s for on-screen preview.
/// On decode failure it returns the last good frame until a keyframe arrives,
/// so no corrupt frames reach the UI. Thread-safe — call `decode(_:)` from any thread.
final class VideoFrameDecoder: Sendable {
  ...
  func decode(_ sampleBuffer: CMSampleBuffer) -> UIImage? {
    ...
    let (session, awaitingKeyframe) = state.withLockUnchecked { state -> (VTDecompressionSession?, Bool) in
      ...
      // Software decoding so the session survives backgrounding.
    ...
    // The handler runs synchronously on this thread because flags is empty.
...
extension CMSampleBuffer {
  /// `kCMSampleAttachmentKey_NotSync` yoksa veya false ise anahtar kare kabul edilir.
